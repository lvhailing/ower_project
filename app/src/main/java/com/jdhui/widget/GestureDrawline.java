...
import com.jdhui.widget.entity.GesturePoint;
...
public class GestureDrawline extends View {
	...
	public GestureDrawline(Context context, List<GesturePoint> list,
		...
		super(context);
		screenDispaly = SystemInfo.getScreenDispaly(context);
		paint = new Paint(Paint.DITHER_FLAG);// 创建一个画笔
		bitmap = Bitmap.createBitmap(screenDispaly[0], screenDispaly[0],
				Bitmap.Config.ARGB_4444); // 设置位图的宽高
		canvas = new Canvas();
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_DOWN:
			...
			if (currentPoint != null) {
				currentPoint
						.setPointState(ApplicationConsts.POINT_STATE_SELECTED);
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (pointAt == null
					...
					|| ApplicationConsts.POINT_STATE_SELECTED == pointAt
							.getPointState()) {
				...
				pointAt.setPointState(ApplicationConsts.POINT_STATE_SELECTED);

				// 判断是否中间点需要选中
				GesturePoint betweenPoint = getBetweenCheckPoint(currentPoint,
						pointAt);
				if (betweenPoint != null
						&& ApplicationConsts.POINT_STATE_SELECTED != betweenPoint
								.getPointState()) {
					// 存在中间点并且没有被选中
	...
	final class clearStateRunnable implements Runnable {
		public void run() {
			...
			for (GesturePoint p : list) {
				p.setPointState(ApplicationConsts.POINT_STATE_NORMAL);
			}
			invalidate();
			isDrawEnable = true;
		}
	...
	private void drawErrorPathTip() {
		...
		for (Pair<GesturePoint, GesturePoint> pair : lineList) {
			pair.first.setPointState(ApplicationConsts.POINT_STATE_WRONG);
			pair.second.setPointState(ApplicationConsts.POINT_STATE_WRONG);
			canvas.drawLine(pair.first.getCenterX(), pair.first.getCenterY(),
					pair.second.getCenterX(), pair.second.getCenterY(), paint);// 画线
		}
		invalidate();
