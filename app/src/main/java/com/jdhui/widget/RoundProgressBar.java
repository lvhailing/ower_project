...
public class RoundProgressBar extends View {
	...
	public RoundProgressBar(Context context, AttributeSet attrs, int defStyle) {
		...
		roundColor = mTypedArray.getColor(
				R.styleable.RoundProgressBar_roundColor, Color.LTGRAY);
	...
	protected void onDraw(Canvas canvas) {
		...
		paint.setColor(getResources().getColor(R.color.light_gray)); // 设置圆环的颜色
		 ...
		 * 中圆环
		 */
		int radiusS = (int) (centre - roundWidth / 2 - ViewUtils.dip2px(
				getContext(), 8)); // 圆环的半径
		...
		float textWidth = paint.measureText(t_center + "%"); // 测量字体宽度，我们需要根据字体的宽度设置在圆环中间
		if (textIsDisplayable && style == STROKE) {
			canvas.drawText(t_center + "",
					centre - textWidth / 2 + ViewUtils.dip2px(getContext(), 4),
					centre + textSize / 2.3f, paint); // 画出进度百分比
		}
		paint.setStyle(Style.STROKE); // 设置空心
		/**
		 * 画圆弧 ，画圆环的进度
		...
		int r = ViewUtils.dip2px(getContext(), 5);
		RectF oval = new RectF(centre - radiusS, centre - radiusS, centre
				+ radiusS, centre + radiusS); // 用于定义的圆弧的形状和大小的界限
