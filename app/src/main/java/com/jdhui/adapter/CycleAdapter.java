...
public class CycleAdapter extends ViewPager {
	 ...
	 *            构造函数
	 */
	public CycleAdapter(Context context, MouldList<ResultCycleIndexBean> images,
		...
		this.setOnPageChangeListener(new OnPageChangeListener() {
			...
			public void onPageSelected(int position) {
				...
				if (dotList.size() != 0) {
					...
					dotList.get(index).setBackgroundResource(
							R.drawable.dot_focus);
					dotList.get(lastPosition).setBackgroundResource(
...
//		}
		...
		public Object instantiateItem(ViewGroup container, int position) {
			...
			if (images.size() > 0) {
				...
				imageLoader.displayImage(images.get(position % images.size())
						.getPicture(), iv, options,
			...
			if (mImageCycleViewListener != null) {
				iv.setOnClickListener(new OnClickListener() {
					...
					public void onClick(View v) {
						mImageCycleViewListener.onImageClick(postItem
								% imageViews.size(), iv);
					}
				});
			}
