...
public class AsyncHttpClient {
	...
	public AsyncHttpClient() {
		...
		HttpConnectionParams.setSoTimeout(httpParams, socketTimeout);// 60000
		HttpConnectionParams.setConnectionTimeout(httpParams, socketTimeout);// 30000
		...
		APNManager manager = APNManager.getInstance();
		boolean mUseWap = manager.isWapNetwork();
		String mProxy = manager.getProxy();
		String mPort = manager.getProxyPort();
		if (mUseWap) {
			HttpHost host = new HttpHost(mProxy, Integer.valueOf(mPort)
					.intValue());
		...
		this.oldTgt = PreferenceUtil.getClienttgt().trim();
		
		
		PersistentCookieStore myCookieStore = new PersistentCookieStore(
				JdehuiApplication.getInstance());
		List<Cookie> cookies = myCookieStore.getCookies();
		BasicCookieStore bcs = new BasicCookieStore();
		bcs.addCookies(cookies.toArray(new Cookie[cookies.size()]));
		httpClient.setCookieStore(bcs);
		 addHttpHeader();
	}

	private void addHttpHeader() {
		// int uid = PreferenceUtil.getUserId();
		int uid = 123456789; // 用户ID
		String imei = SystemInfo.sDeviceId;
		String cookie = PreferenceUtil.getCookie();
		if (uid > 0) {
			addHeader("x-uid", String.valueOf(uid));
		}
		if (!TextUtils.isEmpty(SystemInfo.sVersionName))
