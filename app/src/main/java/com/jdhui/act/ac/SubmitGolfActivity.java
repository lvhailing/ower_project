 ...
 * 服务--高尔夫球场地预约
 */
public class SubmitGolfActivity extends BaseActivity implements View.OnClickListener {
                ...
                if (params != null) {
                    ...
                    if (bookingHospital2B != null) {
                        if (!Boolean.parseBoolean(bookingHospital2B.getFlag())) {
                            Toast.makeText(SubmitGolfActivity.this, "预约成功", Toast.LENGTH_LONG).show();
                            Intent intent = new Intent(SubmitGolfActivity.this, MainActivity.class);
                            intent.putExtra("tab", 2);
                            startActivity(intent);
                            finish();
                        } else {
                            Toast.makeText(SubmitGolfActivity.this, "预约失败，请您检查提交信息", Toast.LENGTH_LONG).show();
                        }
                    } else {
                        Toast.makeText(SubmitGolfActivity.this, "加载失败，请确认网络通畅", Toast.LENGTH_LONG).show();
                    }
                }
            }
        });
...
//            cal.set(Calendar.MILLISECOND, 0);
            ...
            if (currentTime > selectTime) {
                //选择的时间必须是从今天开始包含今天
                Toast.makeText(SubmitGolfActivity.this, "时间只能是今天或今天以后", Toast.LENGTH_SHORT).show();
                return false;
            }
        } catch (ParseException e) {
            e.printStackTrace();
