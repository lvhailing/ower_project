...
public class SubmitHospitalActivity extends BaseActivity implements View.OnClickListener {
    ...
    private void submit() {
        ...
        if (TextUtils.isEmpty(name)) {
            Toast.makeText(SubmitHospitalActivity.this, "请输入预约人", Toast.LENGTH_SHORT).show();
            return;
        }

        if (TextUtils.isEmpty(securityNum)) {
            Toast.makeText(SubmitHospitalActivity.this, "请输入预约人社保号码", Toast.LENGTH_SHORT).show();
            return;
        }

        if (idNo == null || TextUtils.isEmpty(idNo)) {
            Toast.makeText(SubmitHospitalActivity.this, "请输入预约人身份证号", Toast.LENGTH_SHORT).show();
            return;
        } else if (!IdCardCheckUtils.isIdCard(idNo.toUpperCase())) {
            Toast.makeText(SubmitHospitalActivity.this, "请输入正确的身份证号", Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        if (TextUtils.isEmpty(phone)) {
            ...
            Toast.makeText(SubmitHospitalActivity.this, "请输入正确的手机号", Toast.LENGTH_SHORT).show();
            return;
        }

        if (TextUtils.isEmpty(formatTime1)) {
            Toast.makeText(SubmitHospitalActivity.this, "请选择预约时间", Toast.LENGTH_SHORT).show();
            return;
        }

        if (TextUtils.isEmpty(hospitalId)) {
            Toast.makeText(SubmitHospitalActivity.this, "请选择预约医院", Toast.LENGTH_SHORT).show();
            return;
        }

        if (TextUtils.isEmpty(departments)) {
            Toast.makeText(SubmitHospitalActivity.this, "请输入预约科室", Toast.LENGTH_SHORT).show();
            return;
        }
                ...
                if (params != null) {
                    ...
                    if (bookingHospital2B != null) {
                        if (Boolean.parseBoolean(bookingHospital2B.getFlag())) {
                            Toast.makeText(SubmitHospitalActivity.this, "预约成功", Toast.LENGTH_LONG).show();
                            finish();
                        } else {
                            Toast.makeText(SubmitHospitalActivity.this, "预约失败，请您检查提交信息", Toast.LENGTH_LONG).show();
                        }
                    } else {
                        Toast.makeText(SubmitHospitalActivity.this, "加载失败，请确认网络通畅", Toast.LENGTH_LONG).show();
                    }
                }
            }
        });
    ...
    private boolean isTimeValue(String selectedTime) {
        ...
        try {
            ...
            if (currentTime > selectDate.getTime()) {
                //选择的时间必须是从今天开始包含今天
                Toast.makeText(SubmitHospitalActivity.this, "时间只能是今天或今天以后", Toast.LENGTH_SHORT).show();
                return false;
            }
