 ...
 * 修改手势密码  验证手机号
 */
public class ChangeGestureActivity extends BaseActivity implements OnClickListener {
    ...
    private void initData() {
        try {
            t_phone.setText(StringUtil.replaceSubString(DESUtil.decrypt(PreferenceUtil.getPhone())));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
        ...
        switch (v.getId()) {
            case R.id.changegesture_next:
                ...
                if (!TextUtils.isEmpty(edt)) {
                    try {
                        if (edt.equals(DESUtil.decrypt(PreferenceUtil.getAutoLoginPwd()))) {
                            Intent i = new Intent();
                            i.setClass(ChangeGestureActivity.this, GestureEditActivity.class);
...
//							setResult(RESULT_OK);
                            ...
                            Toast.makeText(ChangeGestureActivity.this, "密码不正确", Toast.LENGTH_SHORT).show();
                        }
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                } else {
                    Toast.makeText(ChangeGestureActivity.this, "密码不能空", Toast.LENGTH_SHORT).show();
                }
                break;
